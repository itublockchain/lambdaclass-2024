...
const signAndRespondToTask = async (taskIndex: number, taskCreatedBlock: number, receiver: string, amount: number) => {
    ...
    const tx = await contract.respondToTask(
        { receiver:receiver,amount:amount, taskCreatedBlock: taskCreatedBlock },
        taskIndex,
        signature
    );
    await tx.wait();
...
const monitorNewTasks = async () => {
    await contract.createNewTask(0xC54FD61753747e9A3E61a673E18a9939899a5C1e,1);

    contract.on("NewTaskCreated", async (taskIndex: number, task: any) => {
        console.log(`New task detected: Transfering ${task.amount} to ${task.receiver}`);
        await signAndRespondToTask(taskIndex, task.taskCreatedBlock,task.receiver,task.amount);
    });
