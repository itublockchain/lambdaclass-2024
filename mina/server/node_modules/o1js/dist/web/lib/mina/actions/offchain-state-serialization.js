...
async function fetchMerkleLeaves(contract, config) {
    class MerkleActions extends MerkleList.create(ActionList.provable, (hash, actions) => Actions.updateSequenceState(hash, actions.hash), 
...
async function fetchMerkleMap(height, contract, endActionState) {
    ...
    if ('error' in result)
        throw Error(JSON.stringify(result));
    let leaves = result.map((event) => event.actions
