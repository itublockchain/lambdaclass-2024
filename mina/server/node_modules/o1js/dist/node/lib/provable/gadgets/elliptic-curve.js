...
function verifyEcdsa(Curve, signature, msgHash, publicKey, config = { G: { windowSize: 4 }, P: { windowSize: 4 } }) {
    return verifyEcdsaGeneric(Curve, signature, msgHash, publicKey, (scalars, points, Curve, configs, mode, ia) => multiScalarMul(scalars, points, Curve, configs, mode, ia, true), config);
}
/**
...
function verifyEcdsaV2(Curve, signature, msgHash, publicKey, config = { G: { windowSize: 4 }, P: { windowSize: 3 } }) {
    return verifyEcdsaGeneric(Curve, signature, msgHash, publicKey, (scalars, points, Curve, configs, mode, ia) => multiScalarMul(scalars, points, Curve, configs, mode, ia, false), config);
}
/**
            ...
            if (i % windowSize === 0) {
                ...
                if (hashed) {
                    sjP =
                        windowSize === 1
                            ? points[j]
                            : arrayGetGeneric(HashedPoint.provable, hashedTables[j], sj).unhash();
                }
                else {
                    sjP =
                        windowSize === 1
...
function negateIf(condition, P, f) {
    let y = Provable.if(Bool.Unsafe.fromField(condition), Field3.provable, ForeignField.negate(P.y, f), P.y);
    return { x: P.x, y };
}
...
function decomposeNoRangeCheck(Curve, s) {
    ...
    let lambda = Provable.if(Bool.Unsafe.fromField(s1Negative), Field3.provable, Field3.from(Curve.Scalar.negate(Curve.Endo.scalar)), Field3.from(Curve.Endo.scalar));
    let rhs = Provable.if(Bool.Unsafe.fromField(s0Negative), Field3.provable, ForeignField.Sum(s).add(s0).finish(Curve.order), ForeignField.Sum(s).sub(s0).finish(Curve.order));
